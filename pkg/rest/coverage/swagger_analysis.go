...
func getRESTApi(swaggerPath string, filter string) (map[string]map[string]*RequestStats, error) {
	restAPI := make(map[string]map[string]*RequestStats)
	...
	for _, mp := range document.Analyzer.OperationMethodPaths() {
		...
		if _, ok := restAPI[path]; !ok {
			restAPI[path] = make(map[string]*RequestStats)
		}

		if _, ok := restAPI[path][method]; !ok {
			restAPI[path][method] = &RequestStats{
		...
		addSwaggerParams(restAPI[path][method], document.Analyzer.ParamsFor(method, path), document.Spec().Definitions)
	}
